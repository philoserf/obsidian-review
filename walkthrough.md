...
*2026-03-23T18:21:21Z by Showboat 0.6.1*
<!-- showboat-id: be2ea13a-8e3f-49e1-8429-884e02384b6a -->
...
The plugin is a single-file application (`src/main.ts`, ~560 lines) with one utility module (`src/folderSuggest.ts`). Four exported pure functions handle testable logic; the rest is Obsidian API integration.
...
```output
build.ts (      18 lines)
src/__mocks__/obsidian.ts (      60 lines)
src/folderSuggest.ts (      31 lines)
src/main.test.ts (     107 lines)
src/main.ts (     557 lines)
...
Handles folder renames by updating all reviewed paths under the old prefix. Uses a two-pass approach: collect paths to add in a buffer, then apply them after iteration completes. This avoids mutating the Set while iterating over it in ways that could skip entries.
...
Handles folder deletions by removing all reviewed paths under the folder. Deletes from the Set during iteration — safe per the ECMAScript spec (deleted entries that haven't been visited yet are skipped).
...
A file's review status is derived at runtime — never stored. `getActiveFileStatus` returns `"reviewed"`, `"not_reviewed"`, or `undefined` (for non-markdown or excluded files). This binary model replaced the earlier four-state snapshot model (`to_review`, `reviewed`, `deleted`, `new`).

```bash
sed -n '184,198p' src/main.ts
```
...
The settings tab shows review progress stats, a reset button, excluded folder management with autocomplete, and a status bar toggle. Stats are derived at render time from the vault and reviewed set.

```bash
sed -n '385,397p' src/main.ts
```
...
16 tests cover the four exported pure functions. Tests run via Bun's built-in test runner with an Obsidian mock preloaded.
...
1. **No integration tests.** Only pure functions are tested. Plugin behavior (command guards, event handlers, settings persistence) relies entirely on manual testing in Obsidian.

2. **`reviewedPaths` dual representation.** The runtime `Set<string>` and persisted `string[]` must be kept in sync manually via `saveSettings`. Any mutation to the Set without a corresponding save leaves persisted data stale.

3. **`saveSettings` on every keystroke.** The excluded-folder text input calls `saveSettings` on every `onChange` event (each keystroke). This serializes the entire Set to JSON and writes to disk. At scale (~2,400 reviewed paths), this could cause UI lag. A debounce would help.

4. **`getEligibleFiles` scans the vault on every call.** `openRandomFile` → `getUnreviewedFiles` → `getEligibleFiles` does a full vault scan plus a filter pass. Called every time the user marks a file and opens next. Acceptable at ~2,400 files but won't scale to larger vaults.

5. **`StatusBar.element` and `StatusBar.plugin` are public.** These fields are only used internally but are not marked `private`.
